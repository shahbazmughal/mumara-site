...
const FeaturesBlock04 = () => {
  ...
  // Extract the data from Strapi response
  const build = data.strapiWhychooseus;

  // Function to normalize the URLs for Strapi
  ...
  // Collection for feature items (Advanced Analytics, Expert Support, etc.)
  const collection = [
    {
      text: [
  ...
  return (
    <Container>
      ...
      {build.buttons && (
        <>
          <Divider space={3} />
          <ContentButtons content={build.buttons} />
        </>
      )}
    </Container>
  );
