...
const HeaderBlock01 = () => {
  ...
  const content = {
    ...
    collection: [
      ...
      {
        buttons: [
          { text: 'Contact Us', type: 'PAGE', link: '/contact', variant: 'cta' }
        ]
      }
    ]
  }
      ...
      <Container variant="full" className="nav-container">
        <Container px="4">
          <Flex sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {/* Desktop Nav */}
            <Box sx={{ display: ['none', null, 'block'], flexGrow: 1 }}>
              <Reveal effect="fadeInDown">
                <Flex sx={{ justifyContent: 'flex-end', alignItems: 'center', gap: 4 }}>
                          ...
                          >
                            ...
                            >
                              ...
                              <Box sx={{ bg: 'gray.1', p: 2, borderRadius: '8px' }}>
                                <Box sx={{ fontWeight: 'bold', mb: 2, fontSize: 2, display: 'flex', alignItems: 'center', gap: 2, color: 'text' }}>
                                  {h.Pricing?.servicestitle && <img src={normalize(h.Serviceslogo?.url)} alt={h.Pricing?.servicestitle} width="24" />}
              ...
              <Drawer buttonStyle={{ svg: { size: 32 } }}>
                {content.collection.map((sec, i) => (
                  <Box key={i} sx={{ px: 3, py: 2 }}>
                    ...
                    {sec.label === (h.Pages?.title || 'Pages') ? (
                      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                        <Box>
                          <Box sx={{ fontWeight: 'bold', mb: 1, fontSize: 1, color: 'text' }}>
                            {h.LandingPage?.Heading || 'LANDING PAGES'}
                          </Box>
                          {[
                            { text: h.LandingPage?.software, link: '/saas', icon: h.landinglogo?.url },
                            { text: h.LandingPage?.Softwarev2, link: '/saas-v2', icon: h.landinglogo?.url },
                            { text: h.LandingPage?.fintech, link: '/fintech', icon: h.landinglogo?.url },
                            { text: h.LandingPage?.consulting, link: '/', icon: h.landinglogo?.url },
                            { text: h.LandingPage?.mobile, link: '/app', icon: h.landinglogo?.url },
                            { text: h.LandingPage?.marketing, link: '/marketing', icon: h.landinglogo?.url }
                          ].filter(item => item.text).map((item, k) => (
                            <a
                              key={k}
                              href={item.link}
                              ...
                              style={{ textDecoration: 'none', color: 'inherit' }}
                            >
                              <Flex sx={{ alignItems: 'center', py: 2, gap: 3, fontSize: 1, color: '#2d3748' }}>
                                {item.icon && <img src={normalize(item.icon)} alt={item.text} width="20" />}
                                <span>{item.text}</span>
                              </Flex>
                            </a>
                          ))}
                        ...
                        <Box>
                          <Box sx={{ fontWeight: 'bold', mb: 1, fontSize: 1, display: 'flex', alignItems: 'center', gap: 2, color: 'text' }}>
                            {h.Pricing?.Pricingtitle && <img src={normalize(h.Pricinglogo?.url)} alt={h.Pricing?.Pricingtitle} width="20" />}
                            {h.Pricing?.Pricingtitle || 'PRICING'}
                          </Box>
                          {[
                            { text: h.Pricing?.pricing01, link: '/innerpage/pricing-01' },
                        ...
                        <Box>
                          <Box sx={{ fontWeight: 'bold', mb: 1, fontSize: 1, display: 'flex', alignItems: 'center', gap: 2, color: 'text' }}>
                            {h.Pricing?.servicestitle && <img src={normalize(h.Serviceslogo?.url)} alt={h.Pricing?.servicestitle} width="20" />}
                            {h.Pricing?.servicestitle || 'SERVICES'}
                          </Box>
                          {[
                            { text: h.Pricing?.services01, link: '/innerpage/services-01' },
                        ...
                        <Box>
                          <Box sx={{ fontWeight: 'bold', mb: 1, fontSize: 1, display: 'flex', alignItems: 'center', gap: 2, color: 'text' }}>
                            {h.Pricing?.Abouttitle && <img src={normalize(h.Aboutlogo?.url)} alt={h.Pricing?.Abouttitle} width="20" />}
                            {h.Pricing?.Abouttitle || 'ABOUT US'}
                          </Box>
                          {[
                            { text: h.Pricing?.About01, link: '/innerpage/about-us-01' },
